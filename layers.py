...
def transformation_from_gtmsrtpose(TT, scale,t, invert=False):
    """Convert the coarse pose with predicted pose and scale
    TT: gt pose
    scale: predicted scale factor
    t: predited residue translation
    output: T = [R, t_cp *s + t_NN]
    """
    s =scale.clone()
    t =t.clone()
