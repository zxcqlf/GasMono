...
def mpvit_tiny(**kwargs):
    ...
    checkpoint = torch.load('./networks/ckpt/mpvit_tiny.pth', map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
...
def mpvit_xsmall(**kwargs):
    ...
    checkpoint = torch.load('./networks/ckpt/mpvit_xsmall.pth', map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
...
def mpvit_small(**kwargs):
    ...
    checkpoint = torch.load('./networks/ckpt/mpvit_small.pth', map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
